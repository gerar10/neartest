...
// Implementacion del trait Default para inicializar la estructura de Meme 
...
// Implementacion del metodo new que permite crear nuevos memes 
impl Meme {
...
//1.escritura, state contrato y inicializacion de los valores por default 
#[near_bindgen]
...
impl Default for SimpleMemeMuseum {
    fn default() -> Self {
        Self { 
            //inicializaciamos las colecciones 
...
impl SimpleMemeMuseum {
    ...
    pub fn obtener_memes_museos(&self, nombre_museo: String) -> Vec<Meme> {
        // Obtenemos el museo como un Option<Vec<64>>
        let museo = self.museos.get(&nombre_museo);

         //si el museo existe
         if museo.is_some() {
            // creamos un vector que almacenen la lusta de memes en el museo
            let mut lista_memes = Vec::new(); 
            // el for recurre a cada elemento en el museo
            for meme in &museo.unwrap()  {
                // obtenemos el meme mediante su ID 
                let m = self.memes.get(meme);
                // si el meme existe 
                if m.is_some() {
                    // mandamos el meme el vector a la lista de memes
